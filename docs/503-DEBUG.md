...
## Почему 503 «всегда» на settings, users, news, appeals?

**Причина:** Next.js по умолчанию **префетчит** все ссылки из `<Link>`, видимые во вьюпорте. В сайдбаре дашборда одновременно видны ссылки на «Настройки», «Профсеть», «Новости», «Обращения» и др. При открытии дашборда браузер отправляет **несколько одновременных** GET-запросов вида `/dashboard/settings?_rsc=...`, `/dashboard/users?_rsc=...` и т.д. Каждый такой запрос на сервере выполняет layout (сессия + БД). При нескольких параллельных запросах сервер или nginx не успевает ответить → **503**.

**Что сделано:** для всех ссылок в сайдбаре и мобильном меню выставлен `prefetch={false}`. RSC подгружается только при реальном переходе по ссылке, а не при отображении меню — число одновременных запросов падает, 503 от префетча исчезают.
