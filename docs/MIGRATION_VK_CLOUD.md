...
**Текущая прод-БД:** PostgreSQL в VK Cloud (83.166.237.161, база `myunion_db`). Локальная разработка и сервер приложения (194.87.49.210) подключаются к этой БД через `DATABASE_URL`.
...
3. В настройках доступа добавить IP сервера приложения: **194.87.49.210**.
