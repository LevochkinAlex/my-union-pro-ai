...
if (!sessionId) {
  console.error("❌ Укажите ID сессии: pnpm tsx scripts/fix-session-messages-v2.ts cmij7dj480005ptrczuatcd73");
  process.exit(1);
}
...
async function fixSessionMessages() {
  try {
    ...
    if (allMessages.length > 0) {
      console.log("🗑️  Удаляем все старые сообщения...");
...
[SHOW_SELF_FILL_BUTTON]`,
    ...
    // Проверяем наличие документов (любого статуса кроме DELETED)
    ...
    console.log(`   Есть документы: ${hasAnyDocs ? "✅ ДА" : "❌ НЕТ"}`);
    if (hasAnyDocs) {
      documents.forEach((doc) => {
        console.log(`      - ${doc.type}: ${doc.status} (${doc.filePath ? "есть файл" : "нет файла"})`);
      });
    }
    ...
    if (hasGeneratedDocs && !hasSignedDocs) {
