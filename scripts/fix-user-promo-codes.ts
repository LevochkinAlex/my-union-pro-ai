...
async function fixUserPromoCodes(email: string) {
  try {
    ...
    if (!user.bestBenefitsUserId) {
      ...
      console.log(`   Запустите: pnpm tsx scripts/sync-user-to-bb.ts ${email}`);
      return;
    }
    ...
    claimed.forEach((item: any, idx: number) => {
    ...
    updatedClaimed.forEach((item: any, idx: number) => {
      console.log(`   ${idx + 1}. ID: ${item.id}, PromoCode: ${item.promoCode || "❌ НЕТ"}`);
    });
