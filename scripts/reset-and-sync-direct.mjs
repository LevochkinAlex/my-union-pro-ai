...
/**
 * Прямой вызов функции сброса пароля и синхронизации
 * Работает напрямую с БД, минуя API
 */
...
// Используем DATABASE_URL из .env.local, который указывает на localhost
...
if (!email) {
  console.error('❌ Укажите email: node scripts/reset-and-sync-direct.mjs cursedx@ya.ru');
  process.exit(1);
}
