 ...
 * Проверяет корректность данных: ФИО, адрес, организация, должность, профессия
 */
...
async function testFullFlow() {
  ...
  console.log('='.repeat(80));
  console.log(`
1. ФИО → Должна быть проверка через DaData (склонение)
2. Дата рождения → Проверка формата
3. Адрес → Проверка через DaData (уже работает)
...
- Проверить, что все проверки сработали
  `);
...
4. Проверьте:
   ✓ Адрес проверяется через DaData
