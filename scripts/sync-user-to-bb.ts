...
async function syncUser(email) {
  ...
  try {
    ...
    try {
      ...
      console.log(`   BestBenefits User ID (email): ${user.email}`);
      ...
      console.log(`\n💡 ВАЖНО: BestBenefits API не возвращает user_id.`);
      console.log(`   Используется email как идентификатор: ${user.email}`);
