...
// POST /api/ppo-head/news/upload-image - загрузка изображения для новости председателем
  ...
  try {
    ...
    if (mimeType.startsWith("image/") && mimeType !== "image/gif") {
      try {
        ...
        console.error(`[ppo-head/news/upload-image] Error converting HEIC for ${originalName}:`, error);
      }
    }
    ...
    try {
      ...
      console.log(`[ppo-head/news/upload-image] Image optimized: ${(originalSize / 1024).toFixed(1)}KB -> ${(optimized.size / 1024).toFixed(1)}KB (${optimized.savings}% saved)`);
    } catch (optError) {
      console.error(`[ppo-head/news/upload-image] Optimization failed, using original:`, optError);
    }
    ...
    if (!isVDSStorageConfigured()) {
      ...
      console.log("[ppo-head/news/upload-image] VDS not configured, using base64");
    ...
    try {
      ...
      console.log(`[ppo-head/news/upload-image] Image uploaded to VDS: ${vdsPath}`);
      ...
      console.error("[ppo-head/news/upload-image] VDS upload failed:", vdsError);
      throw new Error("Не удалось загрузить изображение");
    }
  } catch (error) {
    console.error("[ppo-head/news/upload-image] Error:", error);
    return NextResponse.json(
