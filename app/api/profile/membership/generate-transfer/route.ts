...
import { generateMembershipTransferApplication } from "@/lib/pdf/templates/membershipTransferApplication";
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Генерируем PDF
    const pdfBuffer = await generateMembershipTransferApplication(userData);
    ...
    const document = await prisma.document.create({
      data: {
        ...
        type: "MEMBERSHIP_TRANSFER_APPLICATION",
        status: "GENERATED",
        title: "Заявление о переходе в другой профсоюз",
        description: `Заявление о переходе в другой профсоюз от ${userData.lastName} ${userData.firstName}`,
        fileName: `membership_transfer_${user.id}_${Date.now()}.pdf`,
    ...
    await prisma.membershipHistory.create({
      data: {
        ...
        status: "TRANSFERRED",
        statusDate: new Date(),
        notes: "Заявление о переходе в другой профсоюз сгенерировано",
      },
    });
    ...
    console.error("[profile/membership/generate-transfer] POST error:", error);
    return NextResponse.json(
