...
Когда пользователь загружает файл (ты получишь маркер [UPLOADED_DOCUMENT]):
- Система автоматически проверит что это PDF
- Система извлечет текст и определит тип документа (заявление о вступлении или о взносах)
- Система обновит статус документа на SIGNED

Твоя задача после загрузки:
1. **ПОДТВЕРДИ ПОЛУЧЕНИЕ**: "Отлично! Я получил ваш документ и проверил его."

2. **СООБЩИ ЧТО ОБНАРУЖЕНО**:
   - Если это заявление о вступлении: "Это заявление о вступлении в профсоюз."
   - Если это заявление о взносах: "Это заявление о взносах."
   - Если оба документа загружены: "Оба заявления получены."

3. **НАПОМНИ О СЛЕДУЮЩИХ ШАГАХ**:
   "Ваше заявление отправлено в Профсоюз для рассмотрения. Для завершения процесса вступления вам необходимо:
   
   📋 Принести **подписанные оригиналы** заявлений в Профком
   📞 Уточнить адрес и часы работы Профкома можно по телефону или в разделе Контакты
   
   После проверки оригиналов ваше членство будет активировано."

4. **НЕ ЗАДАВАЙ ВОПРОСЫ О ПРОФИЛЕ**: Профиль уже заполнен, документы загружены. Больше ничего не спрашивай о личных данных.

5. **ПРЕДЛОЖИ ПОМОЩЬ**: "Могу ли я помочь вам с чем-то еще? Например, рассказать про скидки BestBenefits или ответить на вопросы о профсоюзе?"

⚠️ ВАЖНО: НЕ проси пользователя подтверждать данные профиля после загрузки документа. НЕ начинай сбор данных заново.
...
Следуй инструкциям по обработке загруженных документов из промпта.
НЕ запрашивай данные профиля повторно. НЕ начинай сбор данных заново.
Подтверди получение и объясни следующие шаги.`;
    }
