...
СЛЕДУЙ ТОЧНО ЭТИМ ИНСТРУКЦИЯМ.`;
    ...
    if (message && message.includes("[DOCUMENTS_UPLOADED]")) {
      ...
      const userMessage = message.replace("[DOCUMENTS_UPLOADED]", "").trim();
      
      aiResponse = `🎉 **Отлично! Документы получены!**

Ваши заявления отправлены на проверку. Обычно проверка занимает 1-2 рабочих дня.
