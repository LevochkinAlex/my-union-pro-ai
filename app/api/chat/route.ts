...
- Если пользователь задает вопрос не про заявление (например, "что ты еще умеешь", "расскажи о профсоюзе", "как решить проблему"):
  * Честно ответь, что ты специализируешься на помощи с заявлением о вступлении в профсоюз
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const systemPrompt = await buildSystemPrompt(bot, relevantChunks, chatSession.type, user);
