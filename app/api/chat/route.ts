      ...
      if (chatSession.type === "STATEMENT" && !hasGeneratedDocuments) {
        try {
          ...
          if (profileIsComplete && user) {
            ...
            // Добавляем маркер завершения и приветственное сообщение
            aiResponse = `🎉 **Поздравляем с успешной регистрацией!**

Ваш профиль заполнен, и документы для вступления в профсоюз успешно сгенерированы.

---

### 📄 Документы готовы к загрузке

На следующем шаге вы сможете:
1. **Скачать** сгенерированные заявления прямо в модальном окне
2. **Распечатать**, подписать и отсканировать их
3. **Загрузить** обратно подписанные документы для отправки на проверку

---

### 🎁 Доступ к эксклюзивным скидкам

Как член профсоюза, вы получаете доступ к **платформе скидок BestBenefits**:

- 🛍️ **Более 1000 партнеров** по всей России
- 💰 **Скидки до 70%** на товары и услуги
- 📱 **Добавьте карту в Apple Wallet** для удобного использования
- 🎫 **Генерируйте промокоды** прямо из приложения
...
- Расскажу о ваших правах и льготах
- Помогу разобраться с вопросами по профсоюзу
...
[PROFILE_COMPLETE]`;
        ...
        aiResponse = `🎉 **Рад видеть вас снова!**

Ваш профиль уже заполнен, и документы готовы.
