...
- Будь терпеливым и понимающим, если пользователь отвечает не так, как ожидается`;
    ...
    // Если заявление уже сгенерировано, добавляем расширенную функциональность
    if (hasGeneratedDocuments) {
      prompt += `\n\n## РАСШИРЕННАЯ ФУНКЦИОНАЛЬНОСТЬ (после генерации заявления):

После того как заявление сгенерировано, ты можешь помочь пользователю с дополнительными вопросами:
