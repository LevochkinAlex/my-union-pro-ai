    ...
    if (preferences && payload.discounts) {
      ...
      claimed.forEach((item: any, index: number) => {
        ...
        if (typeof item === 'object' && item !== null && item.id) {
          ...
          if (promoCode && promoCode.length > 0 && promoCode !== 'null' && promoCode !== 'undefined') {
            ...
            console.log(`[api/discounts] ⚠️ No valid promo code for discount ${discountId} (promoCode: ${item.promoCode})`);
          }
        } else {
      ...
      payload.discounts = payload.discounts.map((discount: any) => {
        ...
        if (savedPromoCode) {
          ...
          // Проверяем, есть ли эта скидка в списке claimed (для отладки)
          const isClaimed = claimed.some((item: any) => {
            const itemId = typeof item === 'object' && item !== null ? String(item.id) : String(item);
            return itemId === discountId;
          });
          
          if (isClaimed) {
            console.log(`[api/discounts] ⚠️ Discount ${discountId} (${discount.title}) is claimed but has no promo code in map`);
          } else {
