        ...
        while ((match = imgRegex.exec(content)) !== null) {
          ...
          if (imageUrl.startsWith("http://") || imageUrl.startsWith("https://") || 
            ...
            try {
              ...
              imageMap.set(imageUrl, finalFilePath);
              console.log(`[posts] Mapped image URL: ${imageUrl} -> ${finalFilePath}`);
            } catch (error) {
        ...
        if (isVDSStorageConfigured()) {
          try {
            ...
            if (vdsPath) {
              // Преобразуем путь в URL для API endpoint
              finalFilePath = vdsPath.replace(/^\/uploads\//, '/api/uploads/');
              console.log(`[posts/PATCH] File uploaded to VDS: ${vdsPath} -> ${finalFilePath}`);
            } else {
