...
export default function OrganizationsPage() {
  ...
  const searchExistingUser = async (email?: string, phone?: string) => {
    ...
    try {
      ...
      const response = await fetch(`/api/org-head/users/search?${params}`);
      if (response.ok) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`/api/org-head/organizations/${selectedOrg.id}`);
  ...
  const loadOrganizations = async () => {
    try {
      ...
      const response = await fetch("/api/org-head/organizations?includeInactive=true");
  ...
  const handleSave = async () => {
    try {
      ...
      const url = isCreating
        ? "/api/org-head/organizations"
        : `/api/org-head/organizations/${selectedOrg?.id}`;
      ...
      if (response.ok) {
        ...
        if (shouldSendInvite) {
          try {
            const inviteResponse = await fetch(`/api/org-head/organizations/${result.organization.id}/invite-chairman`, {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const response = await fetch(`/api/org-head/organizations/${id}`, {
        method: "DELETE",
      });
      ...
      <div className="flex h-full items-center justify-center">
        <div className="text-center max-w-md">
          ...
          <p className="text-gray-600 dark:text-gray-400 mb-4">
            Для доступа к этой странице необходимо войти в систему как руководитель организации.
          </p>
          <a
            href="/login"
            ...
            <div>
              ...
              <div className="flex justify-end gap-3 border-t border-gray-200 px-6 py-4 dark:border-gray-700">
                <button
                  ...
                  className="rounded-lg border border-gray-300 px-4 py-2 font-medium text-gray-700 hover-surface dark:border-gray-600 dark:text-gray-300"
                >
                  Отмена
                </button>
                <button
