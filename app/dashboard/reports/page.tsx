...
const STATUS_BADGE_COLORS: Record<Report["status"], "gray" | "blue" | "orange" | "green"> = {
  DRAFT: "gray",
  SUBMITTED: "blue",
  REVISION: "orange",
  APPROVED: "green",
...
const REPORT_TABS = [
  { id: "reports", label: "Форма отчётности" },
  { id: "tickets", label: "Статистика по обращениям" },
];
...
export default function ReportsPage() {
  ...
  const reportColumns = [
    ...
    },
    {
      ...
      header: "Статус",
      render: (report: Report) => (
        <StatusBadge color={STATUS_BADGE_COLORS[report.status]}>
        ...
        report.deadline ? (
          <div className="flex items-center gap-2">
            ...
            {report.daysLeft !== null && report.daysLeft <= 7 && (
              <span
                className={`text-xs ${
                  report.deadlineStatus === "overdue" ? "text-red-500" : "text-orange-500"
                }`}
              >
                {report.daysLeft < 0
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Отчётность"
        description="Управление отчётами организации"
        actions={
          activeTab === "reports" ? (
            <button
        ...
        <>
          ...
          {filteredReports.length === 0 ? (
            <EmptyState
              ...
              title="Нет отчётов"
              description="Создайте первый отчёт, нажав кнопку выше"
            />
          ) : (
            <DataTable<Report>
      ...
      {activeTab === "tickets" &&
        (ticketStats ? (
              ...
              <Card padding="lg">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span className="text-2xl font-bold text-blue-600">
                    {ticketStats.avgResolutionTime !== null
                      ? `${ticketStats.avgResolutionTime} дн.`
                      : "—"}
                  </span>
                </div>
                <div className="text-sm text-gray-500 dark:text-gray-400">
                ...
                <div className="grid grid-cols-12 gap-2">
                  {Object.entries(ticketStats.byMonth).map(([month, data]) => {
                    const maxTotal = Math.max(
                      ...Object.values(ticketStats.byMonth!).map((d) => d.total),
                      1,
                    );
          ...
          <EmptyState
            ...
            description="Не удалось загрузить статистику по обращениям."
            action={
              <button
                type="button"
                onClick={() => fetchData()}
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-md mx-4 p-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                {templates.length === 0 && (
                  <p
                    id="no-templates-hint"
                    className="mt-2 text-sm text-amber-600 dark:text-amber-400"
                  >
                    Шаблоны отчётов не загружены. Выполните в проекте:{" "}
