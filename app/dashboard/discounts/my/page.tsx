...
export default function MyDiscountsPage() {
  ...
  // Автосинхронизация промокодов
  const { 
    isSyncing, 
    lastSyncResult, 
    sync: manualSync,
    lastSyncTime,
  } = useAutoSyncDiscounts({
    syncOnMount: true,
    onSyncComplete: (result) => {
      console.log("[MyDiscounts] Auto-sync completed:", result);
      if (result.success) {
        loadMyDiscounts();
      }
    },
  ...
  useEffect(() => {
    ...
    // Загружаем скидки (синхронизация происходит автоматически через хук)
    loadMyDiscounts().then(() => {
      hasSyncedRef.current = true;
    });
  }, []);

  // Перезагрузка при смене вкладки
  useEffect(() => {
  ...
  const syncWithBestBenefits = async (force: boolean = false) => {
    ...
    try {
      console.log("[MyDiscounts] Starting manual sync with BestBenefits...", { force });
                ...
                <div className="flex flex-1 flex-col justify-between p-6">
                  ...
                  <div className="mt-4 flex flex-wrap gap-3">
                    <button
                      ...
                      className="flex items-center gap-2 rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover-surface disabled:cursor-not-allowed disabled:opacity-50 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700"
                    >
